# MODELOPT overlay: SupportsEagle3 (DFlash2) + stock quant_config for
# dense/shared MLP. MLA projections remain quant_config=None (BF16).
...
class Glm5NextMoE(nn.Module):
    def __init__(
        ...
        if config.n_shared_experts is None:
            ...
            # ModelOpt W4A16_NVFP4: ignore list keeps shared_experts BF16;
            # pass quant_config so ignore is honored like image stock.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.is_moe and self.num_experts is not None and mlp_type == "sparse":
            ...
            # ModelOpt: dense MLP ignored in quantization_config → BF16 via
            # quant_config+ignore (stock path). Keep Eagle3 on wrapper.
...
class Glm5NextForCausalLM(
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        # Upstream AutoWeightsLoader dropped skip_prefixes; tied lm_head is
        # handled via aliased_params. ignore_unexpected covers leftover embeds.
