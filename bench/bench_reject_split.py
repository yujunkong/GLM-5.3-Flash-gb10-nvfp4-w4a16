...
the probe file written by patches/dflash2_speculator.py when DFLASH2_ACC_PROBE=1:
...
With DFLASH2_ACC_PROBE=0 (Golden), A/B/C stay 0 — expected.
...
# Host side of compose LOG_DIR → /logs mount (this repo default).
_LOG_DIR = os.environ.get("LOG_DIR") or os.path.expanduser("~/logs/glm53")
PROBE_CANDIDATES = [
    Path(_LOG_DIR) / "dflash2-acc-state.json",
    Path("/tmp/dflash2-acc-state.json"),
    Path("/var/tmp/glm53-w4a16-cache/dflash2-acc-state.json"),
    Path("/cache/dflash2-acc-state.json"),
]
...
def get_metrics(api: str) -> dict:
    ...
    for line in text.splitlines():
        ...
        if m:
            ...
            continue
        # Debug gauges (DFLASH2_ACC_PROBE=1 only).
        m = re.match(r"^(dflash2_acc_[a-z_]+)(?:\{[^}]*\})? ([0-9.eE+]+)$", line)
        if m:
